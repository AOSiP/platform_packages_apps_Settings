...
public class ThemeUtils {
    ...
    private static final String[] WHITELISTED_OVERLAYS = { "android.auto_generated_rro__", "com.android.internal.display.cutout.emulation.noCutout" };
    ...
    public static boolean isSubstratumOverlayInstalled(Context mContext) {
        try {
            ...
            for (OverlayInfo overlay : overlayInfoList) {
                if (Arrays.asList(WHITELISTED_OVERLAYS).contains(overlay.packageName))
                    continue;
                if (overlay.state == OverlayInfo.STATE_ENABLED && isSubstratumOverlay(mContext, overlay.packageName))
                    return true;
            }
